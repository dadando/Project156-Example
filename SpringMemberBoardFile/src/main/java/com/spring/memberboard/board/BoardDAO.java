...
public class BoardDAO {
	...
	public ArrayList<BoardVO> getBoardList(int page,int limit) {
		String board_list_sql = "select * from (select rownum rnum,"
				...
				+ "(select * from smemberboard2 order by re_ref desc,re_seq asc)) where "
				+ "rnum>=? and rnum<=?";
	...
	public BoardVO getDetail(BoardVO bvo) throws Exception{
		...
		try {
			...
			if(rs.next()) {
				...
				bvo.setSubject(rs.getString("subject"));
				bvo.setContent(rs.getString("content"));
				bvo.setOrg_file(rs.getString("org_file"));
				bvo.setUp_file(rs.getString("up_file"));
				bvo.setRe_ref(rs.getInt("re_ref"));
				bvo.setRe_lev(rs.getInt("re_lev"));
				bvo.setRe_seq(rs.getInt("re_seq"));
				bvo.setReadcount(rs.getInt("readcount"));
	...
	public int boardInsert(BoardVO bvo) {
		...
		try {
			...
			sql = "insert into smemberboard2(num,id,subject,content,org_file,up_file,re_ref,"
			...
			System.out.println("오리지널:"+bvo.getOrg_file());
			System.out.println("업로드:"+bvo.getUp_file());
			pstmt.setString(5, bvo.getOrg_file());
			pstmt.setString(6, bvo.getUp_file());
			pstmt.setInt(7, num);
			pstmt.setInt(8, 0);
			pstmt.setInt(9, 0);
			pstmt.setInt(10, 0);
	...
	public int boardReply(BoardVO bvo) {
		...
		try {
			...
			sql = "update smemberboard2 set re_seq = re_seq+1 where re_ref=? and re_seq>?";
			pstmt = conn.prepareStatement(sql);
			...
			sql = "insert into smemberboard2(num,id,subject,content,re_ref,"
					+"re_lev,re_seq,readcount,boarddate) values(?,?,?,?,?,?,?,?,sysdate)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setString(2, bvo.getId());
